...
一个函数名字后有const，这个函数必定是成员函数（普通函数后面不能有const）。常成员函数的this指针是常指针，所以不能在常成员函数中修改成员变量。

  
**常成员函数不能更新类的成员变量，但可以使用成员变量**，也不能调用该类中没有用const修饰的成员函数，**只能调用常成员函数**。非常量对象也**可以**调用常成员函数，但是如果有重载的非常成员函数则会优先调用非常成员函数。

- const属性只针对普通成员函数。构造、析构函数，以及一些全局函数，都是没有const属性的
- 非常对象调用常函数，对内存访问的权限缩小，合法。
- 常对象调用非常函数，对内存访问的权限扩大，不合法。
  
#### 作为参数重载

const作为参数时，只有它**修饰的是指针、引用**时，才能作为重载的条件。`fun(char *a)和fun(const char *a)`，这里加了const，const修饰`*a`，也就是说，**不能通过这个指针去改它所指向位置存储的变量**；引用也是同样的道理。

而如果const只是修饰int这种类型数据，由于函数参数值传递，无论函数体里面怎么对int参数修改，都影响不到人家外面的int变量，所以加不加const无所谓。
...
C++中的**常量折叠**：const变量（即常量）值放在编译器的**符号表**中，计算时**编译器直接从表中取值，省去了访问内存的时间**，这是编译器进行的优化。

若要修改const修饰的变量的值，需要加上关键字volatile。若想要在**const成员函数**中修改某些与类状态无关的数据成员，可以使⽤mutable关键字来修饰这个数据成员。
...
int * p = (int *)&age;
*p = 11;
cout<<age<<endl;//39,常量折叠
cout<<*p<<endl;//11
```
- 局部变量 YES
  
如果age是局部变量，可以修改成功，但是由于常量折叠，`age=39`，`*p=11`。age是const变量，编译器对age在**预处理**的时候就进行了替换，编译器只对const变量的值读取一次，age输出是39。

但是由于局部变量存在堆栈，可以可以通过指针修改内存中实际存储的内容，age实际存储的值被指针p修改为11，再去读取`*p`当然就是11了。

- 全局变量 NO

如果age是全局变量，存储在全局空间，且只有可读属性，无法修改其值。

### volatile + 强制类型转化

由于编译器对const修饰的**局部变量**进行了优化，使得对于const变量值的读取实际没有进入内存。而在此基础上加上volatile关键字，即告诉编译器该变量属于易变的，不要对此句进行优化，每次计算时要去**内存**中读取该变量的值，进而避免出现常量折叠的问题。

```C++
const volatile int age = 1;
//强制类型转换
int *p = (int*)&age;  
// or int *p = const_cast<int*>(&age); 
*p = 4;
```

如果不使用强制类型转换，等于是将一个`const int*` 类型赋值给一个`int*` 类型（类似const做参数时的重载情况），属于**从小范围到一个大范围，C++无法进行隐式类型转换**，无法编译通过，所以必须使用强制类型转换。

### mutable

当一个成员变量被声明为mutable时，它显示地表明**这个变量可能在一个const语义环境中被更改**，被mutable修饰的变量，将永远处于可变的状态，即使在一个const函数中。

如果类的成员函数不会改变对象的状态，那么这个成员函数一般会声明成const的。但是有时需要在const的函数里面修改一些跟类状态无关的数据成员，那么这个数据成员就应该被mutalbe来修饰。`mutable int a;`
